...
if (!process.env.SESSION_SECRET) {
  throw new Error("SESSION_SECRET is missing. Set it in your .env or Render environment variables.");
}
...
app.use(
  ...
  app.post("/api/admin/send-license", requireAdmin, async (req, res) => {
    try {
      ...
      const license = await storage.createLicense({
        ...
        plan,
        licenseKey,
        maxDevices: plan === "enterprise" ? 100 : plan === "pro" ? 5 : 2,
        sentAt: new Date(),
      });
  ...
  app.get("/api/download/remaining", async (req, res) => {
    try {
      ...
      res.json({ remaining, total, limit: 20 });
    } catch (error: any) {
      res.status(500).json({ error: error.message });
    }
  });
